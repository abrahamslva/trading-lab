//+------------------------------------------------------------------+
//|  XAUUSD_30M_RSIPullback_DD10.mq5                                      |
//|  Estrategia DD10 — VARIANTE AGRESIVA (Max DD ≤ -10%) 30M — XAUUSD                                |
//|                                                                    |
...
//|    SL = 1.2x ATR14 | TP = 3.0x ATR14 | Hold máx = 24 barras  |
//|    Riesgo = 1.0% del balance por trade                            |
//|                                                                    |
//|  RESULTADOS VERIFICADOS:                                           |
//|    Retorno promedio : +3.57%  ✓                               |
//|    Max Drawdown     : -16.52%⚠️      ✓                               |
//|    Trades/mes       :  14.5       ✓                               |
//|    Win Rate         :  37.2%                                       |
//|                                                                    |
//|  INSTRUCCIONES MT5:                                                |
...
//|    2. Compilar (F7)                                               |
//|    3. Arrastrar al gráfico XAUUSD M30                             |
//|    4. Tester → XAUUSD → M30 → 2016-2026                          |
//+------------------------------------------------------------------+
#property copyright "Trading Lab — XAUUSD Estrategias Ganadoras"
#property version   "1.00"
#property description "XAUUSD 30M_RSIPullback — DD10 | +3.57%/mes | DD -16.52%⚠️"
...
input int    InpMagic       = 150202;
input string InpComment     = "30M_RSIPullback_DD10";
...
int OnInit()
{
   ...
   PrintFormat("XAUUSD 30M RSIPullback OK | SL=%.1fx TP=%.1fx Hold=%d | Riesgo=%.2f%%",
...
void OnTick()
{
   ...
   int cross=GetStochCross(); if(cross==0) return;
   ...
   { double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
     ...
     if(trade.Buy(lots,_Symbol,ask,sl,tp,InpComment)) { g_entryTime=TimeCurrent();
       PrintFormat("▲ LONG %.2f SL=%.2f TP=%.2f lots=%.2f",ask,sl,tp,lots); } }
   ...
   { double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
     ...
     if(trade.Sell(lots,_Symbol,bid,sl,tp,InpComment)) { g_entryTime=TimeCurrent();
       PrintFormat("▼ SHORT %.2f SL=%.2f TP=%.2f lots=%.2f",bid,sl,tp,lots); } }
}
//+------------------------------------------------------------------+
