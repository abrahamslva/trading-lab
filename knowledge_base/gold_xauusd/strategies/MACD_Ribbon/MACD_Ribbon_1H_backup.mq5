...
d t.year, dt.mon, dt.day));
   ...
   int n = 0;
   for(int i = 0; HistoryDealsTotal(); i++)
   {
...
void OnTick()
{
   ...
   if(CopyBuffer(g_hMACD, 0, 1, 3, macdLine) < 3)
   {
      Print(EA_NAME, " MACD line no listo");
      return;
   }
   if(CopyBuffer(g_hMACD, 1, 1, 3, signalLine) < 3)
   {
      Print(EA_NAME, " Signal no listo");
      return;
   }
